...
class MainThread(QThread):
    def __init__(self):
        ...
        print("MainThread __init__ called")
    
    def run(self):
        print("MainThread run() called")
        self.Intro()
    ...
    def run_jarvis(self):
        ...
        while True:
            ...
            print(f"Recognized command in run_jarvis: '{self.command}'")
            ...
            if ('play a song' in self.command) or ('youtube' in self.command) or ("download a song" in self.command) or ("download song" in self.command) : 
                ...
                print(f"yt handler called with command: {self.command}")
            ...
            #Interaction commands with JARVIS
                ...
                print(f"Fun handler called with command: {self.command}")
                self.Fun(self.command)
            elif 'time' in self.command : 
                print(f"Clock_time handler called with command: {self.command}")
                self.Clock_time(self.command)
            # Loosened greeting matching logic
            elif any(greet in self.command for greet in ['hi', 'hai', 'hey', 'hello']):
                print(f"comum handler called with command: {self.command}")
            ...
            #Eg: jarvis sleep the system
                ...
                print(f"Unrecognized command: {self.command}")
                self.talk("Sorry, I did not understand that. Please try again.")
            
    #Intro msg
    def Intro(self):
        print("Intro method called")
        self.talk("Hello, I am JARVIS. Say 'wake up' or 'get up' to start.")
        while True:
            self.permission = self.take_Command()
            print(f"Intro permission: {self.permission}")
            if self.permission == 'None':
                continue
            if ("wake up" in self.permission) or ("get up" in self.permission):
                self.run_jarvis()
                break  # Exit intro loop after running jarvis
            elif ("goodbye" in self.permission) or ("get lost" in self.permission):
                self.talk("Thanks for using me boss, have a good day")
                sys.exit()
            else:
                self.talk("Please say 'wake up' to start or 'goodbye' to exit.")
    ...
    #youtube
    def yt(self,command):
        print(f"yt handler called with command: {command}")
        if 'play' in command:
    ...
    def SendEmail(self,to,content):
        ...
        server.ehlo()
        server.starttls()
        server.login("cybermart2022@gmail.com","iovy epkt urcu uoqq")
        server.sendmail("cybermart2022@gmail.com",to,content)
        server.close()
    ...
    #News
    def news(self):
        MAIN_URL_= "https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=101a8d41c47a460db6c8b2be957c722d"
